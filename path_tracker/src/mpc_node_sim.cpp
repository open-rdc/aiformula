...
void MPCNodeSim::control_loop() {
  ...
  for (int i = 0; i < horizon_; ++i) {
    ...
    reference_trajectory.push_back(
        State{local_path[best_idx].x, local_path[best_idx].y,
              local_path[best_idx].theta, max_v_, 0.0});
  }
...
MPCNodeSim::solve_mpc(const State &current_state,
  ...
  solve_mpc_casadi_c(current_state_arr, ref_traj_arr.data(), N, dt_, L_, max_v_,
