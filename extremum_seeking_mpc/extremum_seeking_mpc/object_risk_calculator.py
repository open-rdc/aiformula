...
from obstacle_detector_msgs.msg import ObjectInfoArray
...
class ObjectRiskCalculator:
    ...
    def init_parameters(self, node: Node):
        node.declare_parameter("object_risk_potential.variance_x")
        self.object_risk_variance_x = node.get_parameter("object_risk_potential.variance_x").value

        node.declare_parameter("object_risk_potential.variance_y")
        self.object_risk_variance_y = node.get_parameter("object_risk_potential.variance_y").value

        node.declare_parameter("object_risk_potential.correlation_coefficient")
        self.object_risk_correlation_coefficient = node.get_parameter("object_risk_potential.correlation_coefficient").value

        node.declare_parameter("object_risk_potential.gain")
        self.object_risk_gain = node.get_parameter("object_risk_potential.gain").value

    def init_connections(self, node: Node, buffer_size):
        self.object_position_sub = node.create_subscription(
            ObjectInfoArray, 'sub_object_info', self.object_info_callback, buffer_size)

    def object_info_callback(self, msg: ObjectInfoArray) -> None:
        with self.lock:
