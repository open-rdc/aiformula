...
PfTargetMap::PfTargetMap (std::vector<PfMapPoint> points) : points_ (std::move (points)), root_index_ (-1) {
...
int PfTargetMap::build_tree (std::vector<std::size_t> &indices, const std::size_t begin, const std::size_t end, const int depth) {
    if (begin >= end) {
        return -1;
    }
    ...
    std::nth_element (indices.begin () + static_cast<std::ptrdiff_t> (begin), indices.begin () + static_cast<std::ptrdiff_t> (middle), indices.begin () + static_cast<std::ptrdiff_t> (end), [this, axis] (const std::size_t lhs, const std::size_t rhs) {
...
bool PfTargetMap::nearest (const Eigen::Vector2d &query, const double max_distance_sq, std::size_t &nearest_index) const {
    if (root_index_ < 0) {
        return false;
    }
...
void PfTargetMap::nearest_recursive (const int node_index, const Eigen::Vector2d &query, std::size_t &nearest_index, double &nearest_distance_sq, bool &found) const {
    if (node_index < 0) {
        return;
    }
...
ParticleFilter::ParticleFilter (const ParticleFilterConfig &config, const std::uint32_t seed) : config_ (config), rng_ (seed) {}

bool ParticleFilter::initialized () const {
    return initialized_;
}

void ParticleFilter::initialize (const double x, const double y, const double yaw, const double position_std, const double yaw_std) {
...
void ParticleFilter::predict (const double linear_velocity, const double yaw_rate, const double dt) {
    ...
    for (auto &particle : particles_) {
        ...
        particle.yaw = normalize_angle (particle.yaw + delta_yaw + sample_gaussian (rng_, rot_std));
    }
}
...
void ParticleFilter::update_weights (const std::vector<Eigen::Vector2d> &source_points_base_link, const PfTargetMap &target_map) {
    ...
    for (std::size_t i = 0U; i < particles_.size (); ++i) {
        ...
        for (const auto &source_point : source_points_base_link) {
            const Eigen::Vector2d transformed (particle.x + cos_yaw * source_point.x () - sin_yaw * source_point.y (), particle.y + sin_yaw * source_point.x () + cos_yaw * source_point.y ());

            std::size_t nearest_index = 0U;
            if (target_map.nearest (transformed, max_distance_sq, nearest_index)) {
                residual_sq_sum += (target_map.point (nearest_index).position - transformed).squaredNorm ();
            } else {
