...
ParticleFilterConfig make_particle_filter_config(rclcpp::Node& node) {
    ...
    config.odom_fw_dev_per_fw           = node.get_parameter("odom_fw_dev_per_fw").as_double();
    config.odom_fw_dev_per_rot          = node.get_parameter("odom_fw_dev_per_rot").as_double();
    config.odom_rot_dev_per_fw          = node.get_parameter("odom_rot_dev_per_fw").as_double();
    config.odom_rot_dev_per_rot         = node.get_parameter("odom_rot_dev_per_rot").as_double();
    config.likelihood_dev               = node.get_parameter("likelihood_dev").as_double();
    config.likelihood_max_dist          = node.get_parameter("likelihood_max_dist").as_double();
    config.resample_ess_ratio_threshold = node.get_parameter("resample_ess_ratio_threshold").as_double();
    config.reinit_residual_threshold    = node.get_parameter("reinit_residual_threshold").as_double();
    config.reinit_consecutive_frames    = static_cast<int>(node.get_parameter("reinit_consecutive_frames").as_int());
...
void fill_pose_covariance(std::array<double, 36>& covariance, const Eigen::Matrix2d& position_covariance, const double yaw_variance) {
    covariance.fill(0.0);
...
PoseEstimaterNode::PoseEstimaterNode(const rclcpp::NodeOptions& options) : PoseEstimaterNode("", options) {}
...
PoseEstimaterNode::PoseEstimaterNode(const std::string& name_space, const rclcpp::NodeOptions& options)
    ...
    initial_pose_subscription_     = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>("/initialpose", rclcpp::QoS(1), std::bind(&PoseEstimaterNode::initial_pose_callback, this, std::placeholders::_1));

    pf_pose_publisher_             = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("/localization/pf_pose", rclcpp::SensorDataQoS().keep_last(1));
    particle_pose_array_publisher_ = this->create_publisher<geometry_msgs::msg::PoseArray>("/localization/particle", rclcpp::QoS(1));
...
bool PoseEstimaterNode::gnss_to_map_pose(const sensor_msgs::msg::NavSatFix& gnss_msg, const sensor_msgs::msg::Imu& imu_msg, geometry_msgs::msg::PoseWithCovarianceStamped& pose_out) const {
    ...
    fill_pose_covariance(pose_out.pose.covariance, Eigen::Matrix2d::Identity() * gnss_position_variance_, imu_yaw_variance_);

    return true;
}

void PoseEstimaterNode::initialize_particle_filter(const geometry_msgs::msg::PoseWithCovarianceStamped& raw_pose) {
    const double raw_yaw = utils::yaw_from_quaternion(raw_pose.pose.pose.orientation);
    particle_filter_.initialize(raw_pose.pose.pose.position.x, raw_pose.pose.pose.position.y, raw_yaw, std::sqrt(gnss_position_variance_), std::sqrt(imu_yaw_variance_));
}

geometry_msgs::msg::PoseWithCovarianceStamped PoseEstimaterNode::make_pose(const builtin_interfaces::msg::Time& stamp, const PoseEstimate2D& estimate) const {
...
void PoseEstimaterNode::timer_callback() {
    ...
    const bool                                    has_raw_pose = gnss_msg && imu_msg && gnss_to_map_pose(*gnss_msg, *imu_msg, raw_pose);

    if (!particle_filter_.initialized()) {
        if (!has_raw_pose) {
            RCLCPP_DEBUG(this->get_logger(), "GNSSとIMUを待機中");
            return;
        }
    ...
    if (has_new_lane_line && map_points && !map_points->empty()) {
        ...
        if (source_points.size() >= min_observed_points_) {
            ...
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "not enough observed lane points: %zu", source_points.size());
        }
    }
