  ...
  has_last_imu_update_stamp_(false)
{
    ...
    gnss_subscription_ = this->create_subscription<sensor_msgs::msg::NavSatFix>(
        "/vectornav/gnss",
        qos_,
        std::bind(&LocalizationNode::gnss_callback, this, std::placeholders::_1));
...
void LocalizationNode::mask_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
    std::lock_guard<std::mutex> lock(data_mutex_);
    latest_mask_image_ = msg;
}

void LocalizationNode::vector_map_callback(const vectormap_msgs::msg::VectorMap::SharedPtr msg)
{
    rebuild_map_points(*msg);
}

void LocalizationNode::gnss_callback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
{
    std::lock_guard<std::mutex> lock(data_mutex_);
    latest_gnss_msg_ = msg;
}

void LocalizationNode::imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
    std::lock_guard<std::mutex> lock(data_mutex_);
    latest_imu_msg_ = msg;
}

void LocalizationNode::velocity_callback(
...
geometry_msgs::msg::PoseWithCovarianceStamped LocalizationNode::update_ekf_with_raw_pose(
...
std::vector<Eigen::Vector2d> LocalizationNode::observed_points_in_initial_map(
