...
DelayGateResult check_delay_gate (const rclcpp::Time &now, const rclcpp::Time &stamp, const double max_delay_s) {
...
EkfLocalizer::EkfLocalizer (const EkfLocalizerConfig &config)
    : config_ (config), state_ (Eigen::Vector3d::Zero ()), covariance_ (Eigen::Matrix3d::Identity ()), stamp_ (0, 0, RCL_ROS_TIME), initialized_ (false), last_position_stamp_ (0, 0, RCL_ROS_TIME), last_yaw_stamp_ (0, 0, RCL_ROS_TIME) {}

bool EkfLocalizer::initialized () const {
    return initialized_;
}

void EkfLocalizer::initialize (const double x, const double y, const double yaw, const rclcpp::Time &stamp) {
...
void EkfLocalizer::predict (const double velocity, const double yaw_rate, const rclcpp::Time &stamp) {
...
bool EkfLocalizer::update_position (const double x, const double y, const Eigen::Matrix2d &covariance, const rclcpp::Time &stamp) {
    ...
    const double dt           = has_last_position_stamp_ ? std::max ((stamp - last_position_stamp_).seconds (), 0.0) : 0.0;
    const bool   force_accept = position_rejected_elapsed_s_ >= config_.position_gate_max_reject_duration_s;

    const bool accepted = apply_position_update (Eigen::Vector2d (x - reference_x, y - reference_y), covariance, force_accept);
...
bool EkfLocalizer::apply_position_update (const Eigen::Vector2d &residual, const Eigen::Matrix2d &covariance, const bool force_accept) {
    Eigen::Matrix<double, 2, 3> observation = Eigen::Matrix<double, 2, 3>::Zero ();
    observation (0, 0)                      = 1.0;
    observation (1, 1)                      = 1.0;

    const Eigen::Matrix2d innovation_covariance = observation * covariance_ * observation.transpose () + covariance;
    ...
    const Eigen::Matrix<double, 3, 2> gain = covariance_ * observation.transpose () * innovation_covariance.inverse ();
...
bool EkfLocalizer::update_yaw (const double yaw, const double variance, const rclcpp::Time &stamp) {
    double reference_x   = state_ (0);
    double reference_y   = state_ (1);
    double reference_yaw = state_ (2);
    pose_at (stamp, reference_x, reference_y, reference_yaw);

    const double dt           = has_last_yaw_stamp_ ? std::max ((stamp - last_yaw_stamp_).seconds (), 0.0) : 0.0;
    const bool   force_accept = yaw_rejected_elapsed_s_ >= config_.yaw_gate_max_reject_duration_s;

    const bool accepted = apply_yaw_update (normalize_angle (yaw - reference_yaw), variance, force_accept);
...
bool EkfLocalizer::apply_yaw_update (const double residual, const double variance, const bool force_accept) {
    Eigen::Matrix<double, 1, 3> observation = Eigen::Matrix<double, 1, 3>::Zero ();
    observation (0, 2)                      = 1.0;

    const double innovation_covariance = (observation * covariance_ * observation.transpose ()) (0, 0) + variance;
    ...
    const Eigen::Vector3d gain = covariance_ * observation.transpose () / innovation_covariance;
...
bool EkfLocalizer::pose_at (const rclcpp::Time &stamp, double &x, double &y, double &yaw) const {
    ...
    const auto upper = std::lower_bound (history_.begin (), history_.end (), stamp, [] (const HistoryEntry &entry, const rclcpp::Time &query) { return entry.stamp < query; });
...
geometry_msgs::msg::PoseWithCovarianceStamped EkfLocalizer::make_pose (const std::string &frame_id) const {
...
void EkfLocalizer::record_history () {
    ...
    while (history_.size () > 1U && (stamp_ - history_.front ().stamp).seconds () > HISTORY_RETENTION_SEC) {
        history_.pop_front ();
    }
}
