  ...
  const Spline & track, const State & x) const
{
  ...
  // 法線ベクトル (経路接線に直交，経路進行方向に対して +左 / -右)
  const double          d_norm = std::hypot(d_center(0), d_center(1));
  const Eigen::Vector2d normal =
    (d_norm > 1e-9) ? Eigen::Vector2d(-d_center(1) / d_norm, d_center(0) / d_norm)
  ...
  // 左境界は +normal 方向 (n_left>0)，右境界は -normal 方向 (n_right>0)
  // それぞれ車体の半幅相当だけコリドーを内側に狭めて運動学的余裕を確保する
  const Eigen::Vector2d pos_left  = pos_center + ( n_left  - 0.5 * projected_size) * normal;
  const Eigen::Vector2d pos_right = pos_center - ( n_right - 0.5 * projected_size) * normal;

  const Eigen::Vector2d car_pos   = {x.X, x.Y};
  const double          cur_proj  = normal.dot(car_pos);

  const double dl = normal.dot(pos_right) - cur_proj;
  const double du = normal.dot(pos_left)  - cur_proj;

  // 接近側 (dl,du のうち絶対値が小さい側) の境界のヘディング感度を線形化に反映する
