...
VectormapGlobalPlannerNode::VectormapGlobalPlannerNode(const rclcpp::NodeOptions& options)
: VectormapGlobalPlannerNode("", options)
{
}

VectormapGlobalPlannerNode::VectormapGlobalPlannerNode(
    const std::string& name_space,
    const rclcpp::NodeOptions& options)
: rclcpp::Node("vectormap_global_planner_node", name_space, options),
  ...
  last_nav_cmd_turn_(vectormap_msgs::msg::LaneConnection::TURN_STRAIGHT)
{
    ...
    vector_map_subscription_ = create_subscription<vectormap_msgs::msg::VectorMap>(
        vector_map_topic_,
        qos_,
        std::bind(&VectormapGlobalPlannerNode::vector_map_callback, this, std::placeholders::_1));
    pose_subscription_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        localization_pose_topic_,
        qos_,
        std::bind(&VectormapGlobalPlannerNode::pose_callback, this, std::placeholders::_1));
    nav_cmd_subscription_ = create_subscription<std_msgs::msg::String>(
        nav_cmd_topic_,
        qos_,
        std::bind(&VectormapGlobalPlannerNode::nav_cmd_callback, this, std::placeholders::_1));
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&VectormapGlobalPlannerNode::timer_callback, this));
}

void VectormapGlobalPlannerNode::vector_map_callback(
...
void VectormapGlobalPlannerNode::pose_callback(
...
void VectormapGlobalPlannerNode::nav_cmd_callback(const std_msgs::msg::String::SharedPtr msg)
{
    if (!msg) {
...
void VectormapGlobalPlannerNode::timer_callback()
{
    std::lock_guard<std::mutex> lock(data_mutex_);
    if (!global_path_ready_ || !latest_pose_) {
        RCLCPP_WARN_THROTTLE(
...
nav_msgs::msg::Path VectormapGlobalPlannerNode::make_global_path_message(
...
geometry_msgs::msg::Quaternion VectormapGlobalPlannerNode::yaw_to_quaternion(const double yaw)
{
