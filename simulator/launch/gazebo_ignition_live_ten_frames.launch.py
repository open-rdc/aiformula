...
def _spawn_gemini_live_ten_frames_node(context) -> List[ExecuteProcess]:
    ...
    cmd = [
        'python3',
        '-m',
        'gemini_cmd_vel.gemini_live_controller_node_ten_frames',
        '--ros-args',
    ]
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                ...
                description='Google Gemini の API キー。空欄にするとリクエストを送信しません。',
            ),
            DeclareLaunchArgument(
                'gemini_model',
                default_value=EnvironmentVariable('GEMINI_MODEL', default_value=''),
                description=(
                    'Gemini Live API で使用するモデル名。短縮名（推奨）または models/... 形式の完全名を指定。'
                ),
            ),
            IncludeLaunchDescription(
            ...
            bridge,
            OpaqueFunction(function=_spawn_gemini_live_ten_frames_node),
        ]
    )
