...
std::vector<cv::Rect> ObstacleDetectorNode::detect_red_objects(const cv::Mat& image)
{
  ...
  cv::Mat erode_kernel = cv::getStructuringElement(cv::MORPH_RECT,
    cv::Size(red_params_.erode_kernel_size, red_params_.erode_kernel_size));
  cv::Mat dilate_kernel = cv::getStructuringElement(cv::MORPH_RECT,
    cv::Size(red_params_.dilate_kernel_size, red_params_.dilate_kernel_size));

  cv::erode(red_mask, red_mask, erode_kernel, cv::Point(-1, -1), red_params_.erode_iterations);
  cv::dilate(red_mask, red_mask, dilate_kernel, cv::Point(-1, -1), red_params_.dilate_iterations);
...
void ObstacleDetectorNode::publish_result_image(const cv::Mat& image, const std::vector<cv::Rect>& bboxes, const std_msgs::msg::Header& header)
{
  ...
  try {
    ...
    result_image_publisher_->publish(*result_cv_image.toImageMsg());
  } catch (cv_bridge::Exception& e) {
